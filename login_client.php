...
$query = "SELECT client_username, client_password,auth FROM clients WHERE client_username=? AND client_password=? LIMIT 1";

// To protect MySQL injection for Security purpose
$stmt = $conn->prepare($query);
$stmt -> bind_param("ss", $client_username, $client_password);
$stmt -> execute();
$stmt -> bind_result($client_username, $client_password,$auth);
$stmt -> store_result();
